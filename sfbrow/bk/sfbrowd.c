...
store names, path as both string and runes throughout
...
//		print("keyboardthread read unhandled rune %x\n", r);
	}
}
	...
	Rune *filterrs)
{
	...
	// todo: filter ents according to filterrs
	return dispsel;
}
...
statethread(void *)
{
	...
	for(;;) {
		...
		case SEL:
			...
			drawall(layout, currsel, scrollidx, filterrs);
			freesel(dispsel);
			break;
		case SORT:
			if(makingsel)
			...
			drawnamepanel(layout, currsel, scrollidx, filterrs);
			freesel(dispsel);
			break;
		case ENTSCRIPT:
			if(makingsel)
