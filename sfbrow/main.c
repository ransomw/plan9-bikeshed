...
bug: filter filenames from root directory.
	read: "canonicalization of pathnames"
		linked also from golang docs
...
statethread(void *)
{
	...
	appState.scrollidx = 0;
	appState.currsel = nil;
	appState.makingsel = 0;
	...
	for(;;) {
		...
		sortcompfn = sortaction2compfn(appState.sort);
		switch(altidx) {
		case SCROLL:
			if(appState.currsel == nil)
				continue;
			dispsel = makedispsel(appState.currsel, sortcompfn, appState.filterrs);
			if(scrollupdate == SCROLL_UP) {
				if(appState.scrollidx > 0)
					appState.scrollidx--;
			} else if(scrollupdate == SCROLL_DOWN) {
				if(appState.scrollidx < lensel(dispsel) - 1)
					appState.scrollidx++;
			} else if(0 <= scrollupdate && scrollupdate <= 100) {
				appState.scrollidx = scrollupdate * lensel(dispsel) / 100;
			}
			drawnamepanel(appState.layout, dispsel, appState.scrollidx, appState.filterrs);
			freesel(dispsel);
			break;
		case NAME_IDX:
			if(appState.makingsel)
				break;
			if(appState.currsel == nil)
				continue;
			dispsel = makedispsel(appState.currsel, sortcompfn, appState.filterrs);
			nameent = firstdisplayedselent(dispsel->ents, appState.scrollidx);
			while(nameidx > 0 && nameent != nil) {
				nameidx--;
				nameent = nameent->next;
			}
			if(nameent != nil) {
				appState.makingsel = 1;
		...
		case SEL:
			if(newsel != nil) { /* not a resize */
				// todo: error if not appState.makingsel
				if(appState.currsel != nil) {
					if(strcmp(appState.currsel->dirpath, newsel->dirpath) != 0)
						appState.scrollidx = 0;
					freesel(appState.currsel);
				} else
					appState.scrollidx = 0;
				appState.currsel = newsel;
				appState.makingsel = 0;
				*appState.filterrs = '\0';
			}
			dispsel = makedispsel(appState.currsel, sortcompfn, appState.filterrs);
			drawall(computelayout(dispsel, appState.filterrs), dispsel, appState.scrollidx, appState.filterrs);
			freesel(dispsel);
			break;
		case SORT:
			if(appState.makingsel)
			...
			appState.scrollidx = 0;
			dispsel = makedispsel(appState.currsel, sortcompfn, appState.filterrs);
			// sorting the entries list doesn't change the appState.layout
			drawnamepanel(appState.layout, dispsel, appState.scrollidx, appState.filterrs);
			freesel(dispsel);
			break;
		case ENTSCRIPT:
			if(appState.makingsel)
				break;
			runentscript(appState.currsel->path, entscript);
			break;
		case FILTER:
			updatefilterrs(appState.filterrs, filterr);
			dispsel = makedispsel(appState.currsel, sortcompfn, appState.filterrs);
			drawnamepanel(computelayout(dispsel, appState.filterrs), dispsel, appState.scrollidx, appState.filterrs);
			freesel(dispsel);
			break;
		case LAYOUT:
			sendp(layoutresc, &appState.layout);
			break;
		}
	}
}
